  ...
  public class EditWorkTimeDayJobRequestValidator : BaseEditRequestValidator<EditWorkTimeDayJobRequest>, IEditWorkTimeDayJobRequestValidator
    {
        ...
        private void HandleInternalPropertyValidation(Operation<EditWorkTimeDayJobRequest> requestedOperation, CustomContext context)
        {
            Context = context;
            ...
            AddFailureForPropertyIf(
                ...
                {
                    {
                        x => Guid.TryParse(x.value.ToString(), out Guid id) && _repository.Contains(id),
